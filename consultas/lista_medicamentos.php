...
<?php
				...
				$stmt = $mysqli->prepare('SELECT iddetalhe,dosagem,precriçao,utilizador.nome as nome_utili,utentes.nome as nome_ute FROM detalhesconsuta INNER JOIN utilizador ON utilizador.idutilizador = detalhesconsuta.idutilizador INNER JOIN utentes ON utentes.idutentes = detalhesconsuta.idutente   '); 
				...
				while($row = $result->fetch_assoc()) {
					  echo"<tr>";
						  echo"<td>".$row['iddetalhe']."</td>";
						  echo"<td>".$row['nome_utili']."</td>";
						  echo"<td>".$row['nome_ute']."</td>";
						  echo"<td>".$row['precriçao']."</td>";
						  echo"<td>".$row['dosagem']."</td>";
						  
						  
						  echo'<td><button type="button" class="btn btn-sm btn-primary" data-toggle="modal" data-target="#modadetalhe"
							  
							  data-idpri='.$row['iddetalhe'].'
...
<div class="modal fade" id="modadetalhe" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <span aria-hidden="true">&times;</span></button>
					  ...
					  <div class="modal-body">
						<form action="elimina_m.php" method="POST">
